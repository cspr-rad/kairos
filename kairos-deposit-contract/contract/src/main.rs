...
use casper_types::{
    ...
    AccessRights, ApiError, CLType, CLValue, Key, Parameter, URef, U512,
};
...
pub extern "C" fn update_admin() {
    let caller = Key::from(get_second_caller_address().unwrap_or_revert());
...
pub extern "C" fn call() {
    /*
        Mandatory named_keys:
            ...
            * kairos_deposit_purse: URef
    */
    let caller = Key::from(get_immediate_caller_address().unwrap_or_revert());
    ...
    let contract_hash_key = Key::contract_entity_key(contract_hash);
